...
#### Separar as categorías em um coluna
```{r}
opiniao <- rep(c(0,1),c(3,3))
opiniao

regiao <- c(rep(c("U","S","R"),c(1,1,1)),rep(c("U","S","R"),c(1,1,1)))
regiao


contagem <- c(30,35,35,60,25,15)
contagem


#regiao <- c(rep(c("urbano","suburbano", "rural"),c(30,35,35)), rep(c("urbano","suburbano", "rural"),c(60,25,15)))
...
#### Gerar o data.frame
```{r}
dados <- data.frame(opiniao, regiao, contagem)
dados
...
modelo <- glm(opiniao ~ regiao + contagem, dados, family = binomial)
...
variancia_amostra
```
## Teste de Hipótese para Homogeneidade de Variâncias

## 1. Entre Gerente e Pesquisa
...
```{r}
f_cal <- variancia_amostra[1]/variancia_amostra[2]
f_cal
```
### Região crítica
```{r}
f1 <- qf(0.025,5,5,lower.tail = FALSE)
f0 <- 1/f1
f0


intervalo_critico <- c(f0,f1)
intervalo_critico
```
### Conclusão

Como fcal =2,667 não pertence a região crítica, não rejeitamos a H0, ao nível de 5% de significância, logo as variâncias são desconhecidas e **iguais**.

## 1. Entre Gerente e Publicidade
...
```{r}
f_cal <- variancia_amostra[1]/ variancia_amostra[3]
f_cal
```
### Anova
```{r}
